...
import { HabbitStateContext, UserStateContext } from "../App";

export default function MissionNew() {
  const { habbitInfo, setHabbitInfo } = useContext(HabbitStateContext);
  const { userInfo, setUserInfo } = useContext(UserStateContext);

  // ✅ `localStorage`에서 `userInfo` 불러오기 (새로고침 후에도 유지되도록)
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    setHabbitInfo([...habbitInfo, newHabit]);
    localStorage.setItem(
      "habbitInfo",
      JSON.stringify([...habbitInfo, newHabit])
    );
    ...
    if (userInfo) {
      ...
      try {
        const response = await fetch("/habit/register", {
          method: "POST",
